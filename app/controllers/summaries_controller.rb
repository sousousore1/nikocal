class SummariesController < ApplicationController
  ...
  def index
    ...
    @stamp_of_year_json = convert_to_pie_json_from @stamp_of_year

    @stamp_of_month = Stamp.all.find_all do |x|
      x.target_date.month == Date.today.month
    end
    @stamp_of_month_json = convert_to_pie_json_from @stamp_of_month
    ...
    @stamp_of_week_json = convert_to_pie_json_from @stamp_of_week

    @stamp_of_today = Stamp.all.find_all do |x|
      x.target_date == Date.today
    end
    @stamp_of_today_json = convert_to_pie_json_from @stamp_of_today
  end

  private

  def convert_to_pie_json_from(stamps)
